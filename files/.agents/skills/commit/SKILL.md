...
- `type`: one of `feat` `fix` `docs` `refactor` `chore` `test` `perf` `build` `ci` `style`. Mark a
  breaking change with `!` (for example `feat(api)!: ...`).
- `scope`: the context or module that changed, named as this repository already names it in
  `git log`.
...
- Commit on the current branch. Do not create or switch branches unless asked.
- Leave generated and derived artifacts out of the commit when the repository keeps them untracked.
