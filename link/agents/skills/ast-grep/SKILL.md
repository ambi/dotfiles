---
name: ast-grep
description: Structural code search and rewriting with ast-grep, including how to write its rules. Use the moment you are about to put a newline, \s+, or a brace into a regex that matches code; when the same replacement has to be written again because only an identifier differs between the sites; when a match depends on nesting, such as inside a function, a struct literal, or a test body; or when asked for an AST-based search. Prefer it over multi-line sd, perl, or sed regexes in those cases. Plain rg stays right for locating a symbol to read or edit.
---
...
Vendored from [ast-grep/agent-skill](https://github.com/ast-grep/agent-skill) at `6b668aa`. The body is
unmodified; the frontmatter `description` was rewritten to trigger on observable signs of a structural edit
rather than on the reader having already classified the task as structural, and this note was added.
Upstream README, License: "This skill follows ast-grep's MIT license for any included documentation or examples."
